...
for (name in target_names_plots$name) {
  ...
  p <- tar_read_raw(name)
  p[[1]]$data <- p[[1]]$data[type!="Inverse Probability Weighting"]
  levels(p[[1]]$data$type)[levels(p[[1]]$data$type) == "Debiased ICE-IPCW"] <- "ICE-IPCW (Debiased)"
  levels(p[[1]]$data$type)[levels(p[[1]]$data$type) == "LTMLE (grid size = 8)"] <- "LTMLE"
  levels(p[[2]]$data$type)[levels(p[[2]]$data$type) == "Debiased ICE-IPCW"] <- "ICE-IPCW (Debiased)"
  levels(p[[2]]$data$type)[levels(p[[2]]$data$type) == "LTMLE (grid size = 8)"] <- "LTMLE"
...
target_names_tables <- tar_manifest() |>
  select(name) |>
  filter(grepl("results_table_", name))

for (name in target_names_tables$name) {
  message("Saving table for: ", name)
  df <- tar_read_raw(name)
  df <- df[type!="Inverse Probability Weighting"]
  levels(df$type)[levels(df$type) == "Debiased ICE-IPCW"] <- "ICE-IPCW (Debiased)"
  levels(df$type)[levels(df$type) == "LTMLE (grid size = 8)"] <- "LTMLE"
