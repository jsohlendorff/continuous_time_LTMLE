...
data_continuous_intervention <- simulate_simple_continuous_time_data(
  n = n_true_value,
...
# A new event type "M" is introduced, which denotes the date at which medicine is purchased
# This event will occur jif the patient is treated (A==1) and up to a few days before the patient does not have any medicine left
# When the doctor does not treat the patient (A==0), the patient will not purchase medicine and will dispose of any remaining medicine
# Also, there is no effect of purchasing the medicine, only on taking it
# The prescription includes pills for, say, 200 days
set.seed(1234)
data_continuous <- simulate_continuous_time_purchase_history_data(
  n = n,
  no_competing_events = TRUE,
  effects = vary_effects_simple(),
  uncensored = TRUE,
  days_for_medicine = 200, ## How many days can the patient take the medicine?
  keep_A = TRUE ## Keep the treatment variable A in the data (only for comparison with full access to treatment)
)
...
## Remove A from data; then reconstruct A from purchase history
data_continuous_M <- data_continuous
data_continuous_M$timevarying_data <- data_continuous_M$timevarying_data[event != "A"]
data_continuous_M$timevarying_data[, A:= NULL] 
data_continuous_M <- reconstruct_A_from_purchase_history(data_continuous,
  med_length = 205, ## Allow little bit of extra time for the patient to fetch the medicine
)
debias_ice_ipcw(
  data = copy(data_continuous_M),
  tau = tau,
  model_pseudo_outcome = "quasibinomial",
  model_treatment = "learn_glm_logistic",
  model_hazard = NULL,
