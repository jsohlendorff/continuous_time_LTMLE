...
influence_curve_censoring_martingale_time_varying <- function(dt,
  ...
  #my_dt[,.(Q[.N], F1[.N]),by="id"]
  # browser()
  #
  # setkey(my_dt, time_1, id)
  # levels(dt$event) <- c(3,4,0,1,2)
  # dt$event <- as.numeric(as.character(dt$event))
  # m_event <-  CSC(Hist(time, event) ~ A_0+L_0, data = dt)
  # predict_cumulative_invidence_csc <- function(m, dt, cause = 1) {
  #   browser()
  #   times <- sort(unique(dt$time_1))
  #   dt <- dt[,-"time_1"]
  #   ## remove duplicates
  #   dt <- unique(dt[, c("id","L_0","A_0")])
  #   ## predict cumulative incidence function
  #   c(predictRisk(m,newdata = dt, times = times, cause = cause))
  # }
  # my_dt <- my_dt[, F1 := predict_cumulative_invidence_csc(m_event, .SD, cause = 1)]
  ...
  # levels(dt$event) <- c(3,4,0,1,2)
  # d$event <- as.numeric(as.character(d$event))
  # m_event <-  CSC(Hist(time, event) ~ A_0+L_0, data = dt)
  # predict_cumulative_invidence_csc <- function(m, dt, cause = 1) {
  #   browser()
...
#   ## remove the i.s from the my_:dt after rolling join
#   # my_dt[, grep("i.", names(my_dt)) := NULL]
#   my_dt[is.na(Q), Q := 0]
#
#   ## predict cumulative hazard function
#   my_dt <- cumulative_hazard_cox(learn_censor, my_dt, my_covariate_dt, cens_dt_original, "C")
