...
list(
  ...
  tar_target(
    ...
    clean_outcome(dt_outcome, dt_index, 
                  event_of_interest = "primary.outcome") # MACE is the event of interest
  ),
  ## Combine cleaned into a single data.table in the long format for continuous-time debias ICE-IPCW estimation
  tar_target(
    combined_data_lira,
    combine(comedication_cleaned, adverse_events_cleaned, regime_cleaned, outcome_cleaned, outcomes = c("all_cause_mortality", "mace", "censored"), 
            treat_names = c("lira", "placebo"), id_regimen = id_regimen_lira)
  ),
  tar_target(
    combined_data_placebo,
    combine(comedication_cleaned, adverse_events_cleaned, regime_cleaned, outcome_cleaned, outcomes = c("all_cause_mortality", "mace", "censored"), 
            treat_names = c("lira", "placebo"), id_regimen = id_regimen_placebo)
  ),
  ## Summarize the data;
