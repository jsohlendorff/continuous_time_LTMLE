...
#' @export
learn_glmnet_2 <- function(character_formula,
                         data,
                         intervened_data = data,
    ...
    if (selector == "undersmooth"){
        ...
        args <- c(list(cv = TRUE),list(formula = character_formula,data = model_frame,family = "binomial",...))
        args <- args[unique(names(args))]
        fit <- do.call(glm_net,args)
...
## learn_scaled_glmnet <-
##     function(character_formula, data, outcome_name, max_weight) {
##         data$f_weight <- data[[outcome_name]] / max_weight
##       fit <- glm(
##         as.formula(paste0(
