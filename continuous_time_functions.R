...
test_fun_method_2 <- function(d, tau = 0.02, model_type = "glm") {
  ...
  # table(na.omit(d$event_1))
  # table(na.omit(d$event_2))
  # table(na.omit(d$event_3))
  ...
  # baseline_data <- copy(at_risk_interevent)[, c(baseline_covariates, "p"), with = FALSE]
  # if (model_type == "glm"){
  #   approximate_baseline <- glm(
  #     as.formula(paste0("p ~ ", paste(baseline_covariates, collapse = "+"))),
  #     data = baseline_data,
  #     family = quasibinomial
  #   )
  # } else {
  #   approximate_baseline <- ranger::ranger(
  #     as.formula(paste0("p ~ ", paste(baseline_covariates, collapse = "+"))),
  #     data = baseline_data
  #   )
  # }
  # baseline_data$A_0 <- 1
  # p_1<-mean(predict_function(approximate_baseline, newdata = baseline_data, model_type))
  # p_1
  # at_risk_interevent$A_0 <- 1
  # pp_1<-mean(predict(
  #   learn_cause_specific_cox,
  ...
  # d<- d[,-c("id", "A_1", "L_1")]
  # browser()
  # levels(d$event_1) <- sapply(levels(d$event_1), map_levels)
  # d$event_1 <- as.numeric(as.character(d$event_1))
  # d$A_0 <- as.factor(d$A_0)
  # d$sex <- as.factor(d$sex)
  # d$L_0 <- as.factor(d$L_0)
  # ## change time_1, event_1 to time, event
  # setnames(d, c("time_1", "event_1"), c("time", "event"))
  # #ate_riskRegression <- riskRegression::ate(learn_cause_specific_cox, data = d, treatment times = tau)
  # csc_1 <- CSC(Hist(time, event) ~ age + sex + L_0 + A_0, data = d,cause=1)
  # z<-riskRegression::ate(csc_1, data = d, cause = 1, times = tau,treatment = "A_0")
  # browser()
  # list(p_1=p_1,pp_1=pp_1, ate_riskRegression = )
  ##Going back to IPW
}
...
#
# test_event_1 <- function(d, tau = 0.02, model_type = "glm") {
#   d_res <- widen_continuous_data(d)
#   last_event_number <- d_res$last_event_number
#   d <- d_res$d_wide
#
...
#
#     if (k == 2){
#       learn_cause_specific_cox_2 <- riskRegression::CSC(form, data = at_risk_interevent)
#       at_risk_interevent <- at_risk_interevent[which_before_tau, ]
#       at_risk_interevent <- at_risk_interevent[order(get(paste0("time_", k))), ]
#       ## First learn L part for k=1
#       p_2 <- c(
#         predict(
#           learn_cause_specific_cox_2,
#           newdata = at_risk_interevent,
#           cause = 1,
#           times = tau - at_risk_interevent[[paste0("time_", k - 1)]],
#           diag = TRUE
...
#       at_risk_interevent <- at_risk_interevent[which_before_tau, ]
#       at_risk_interevent <- at_risk_interevent[order(get(paste0("time_", k))), ]
#       at_risk_interevent_L <- at_risk_interevent[get(paste0("event_", k)) == 4 & get(paste0("time_", k)) < tau]
#       at_risk_interevent_A <- at_risk_interevent[get(paste0("event_", k)) == 3 & get(paste0("time_", k)) < tau]
#       p_l <- weighted_cumulative_incidence(
#         learn_cause_specific_cox,
#         approximate_p_L,
#         at_risk_interevent,
...
#
#       at_risk_interevent$A_0 <- 1
#       at_risk_interevent$A_1 <- 1
#       dd <- list()
#       for (i in 1:nrow(at_risk_interevent)){
#         if (at_risk_interevent$event_1[i] == 3 & at_risk_interevent$time_1[i] < tau){
#           dd[[i]] <- at_risk_interevent[,c("A_0","L_0", "L_1","age","sex")]
#           dd[[i]]$time_1 <- at_risk_interevent$time_1[i]
#           dd[[i]]$event_1 <- "A"
#           dd[[i]]$A_1 <- 1
#           dd[[i]]$times <- tau - at_risk_interevent$time_1[i]
#           dd[[i]]$id <- 1:nrow(dd[[i]])
#         }
#       }
#       res <- rbindlist(dd)
#       p_a <-
#         as.data.table(predict(
#           learn_cause_specific_cox_2,
#           newdata = res,
#           cause = 1,
#           times = res$times,
#           diag = TRUE
#         ))
#       p_a$observation <- res$id
#       p_aa <- weighted_cumulative_incidence(
#         learn_cause_specific_cox,
#         approximate_p_A,
#         at_risk_interevent,
#         at_risk_interevent_A,
...
#         tes_dt = p_a
#       )
#       p_a_y <- as.data.table(predict(
#         learn_cause_specific_cox,
#         newdata = at_risk_interevent,
#         cause = 3,
#         times = tau - at_risk_interevent$time_1,
#         diag=TRUE
#       ))
#       p_a_y$diff_cum_inc <- diff(c(0, p_a_y$absRisk))
#       merg
#
#
#
#       ## For each observation add new data set corresponding to baseline value.
#       ## This should i.e., from a 2000 observation data set to a 2000 x 2000 data set
#
#
#       d<-at_risk_interevent[]
#       browser()
#     }
#
#     # if (!first_event) {
